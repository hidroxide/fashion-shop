    ...
    const getOrderItem = async () => {
      try {
        ...
        const result = await axios.get(
          homeAPI + `/order/admin/detail/${id_order}`
        );
  ...
  return (
    <div className="order-detail-page">
      ...
      <div className="header-order-detail-page">
        ...
        <p className="">Ngày đặt hàng {formatTime(orderDetail.created_at)}</p>
      </div>
      ...
      <div className="footer-order-detail-page">
        <div className="row">
          <div className="col-6">
            <div>
              <p className="fw-bold heading_order_histories">
                Lịch sử đơn hàng
              </p>
            </div>
            <div>
              <ul>
                {orderDetail.order_histories &&
                  orderDetail.order_histories.map((item, index) => {
                    return (
                      <li key={index}>
                        {`${formatAllInDate(item.created_at)}: ${
                          item.state_name
                        }`}
                      </li>
                    );
            ...
            <div>
              <table className="">
                <tbody>
                  ...
                  <tr className="row">
                    <td className="col-4">Số điện thoại</td>
                    <td className="col-8 fw-bold d-flex justify-content-end text-end">
