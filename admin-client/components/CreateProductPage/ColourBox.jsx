...
const ColourBox = ({
  ...
  const handleOnDeselect = (colourId, { value }) => {
    ...
    selectedColoursClone = selectedColoursClone.filter(
      (colour) => colour.colour_id != colourId
    );
    setSelectedColours(selectedColoursClone);
    let colourBoxValueClone = [...colourBoxValue];
    colourBoxValueClone = colourBoxValueClone.filter(
  ...
  return (
    <div>
      <label htmlFor="enter-color" className="fw-bold">
        Màu:
      </label>
      <Select
        id="enter-color"
