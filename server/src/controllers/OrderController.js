...
let create = async (req, res, next) => {
  ...
  if (customer_name === undefined)
    return res.status(400).send("Trường customer_name không tồn tại");
  let email = req.body.email;
  if (email === undefined)
    return res.status(400).send("Trường email không tồn tại");
  let phone_number = req.body.phone_number;
  if (phone_number === undefined)
    return res.status(400).send("Trường phone_number không tồn tại");
  let address = req.body.address;
  if (address === undefined)
    return res.status(400).send("Trường address không tồn tại");
  let order_items = req.body.order_items;
  if (order_items === undefined)
    return res.status(400).send("Trường order_items không tồn tại");
  ...
  try {
    ...
    for (let i = 0; i < order_items.length; i++) {
      ...
      let productVariantPrice =
        product_variant.Product.Product_Price_Histories[0].price;
    ...
    let state = await Order_State.findOne({
...
let listAdminSide = async (req, res, next) => {
  try {
    let orderList = await Order.findAll({
      ...
      order: [[Order_Status_Change_History, "created_at", "DESC"]],
    });
...
let detailCustomerSide = async (req, res, next) => {
  ...
  if (order_id === undefined)
    return res.status(400).send("Trường order_id không tồn tại");
  let order;
  try {
...
let changeStatus = async (req, res, next) => {
  ...
  if (state_id === undefined)
    return res.status(400).send("Trường state_id không tồn tại");
  let order;
  try {
  ...
  try {
    ...
    if (state_id == 5) {
      ...
      if (
        stateList.some(even) &&
        stateList[lastIndex].state_id != 4 &&
        stateList[lastIndex].state_id != 6
      ) {
    ...
    if (state_id == 6) {
      ...
      if (
        stateList.some(even) &&
        stateList[lastIndex].state_id != 4 &&
        stateList[lastIndex].state_id != 5
      ) {
