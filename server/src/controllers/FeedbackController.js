...
let create = async (req, res, next) => {
  ...
  if (rate === undefined)
    return res.status(400).send("Trường rate không tồn tại");
  let content = req.body.content;
  if (content === undefined)
    return res.status(400).send("Trường content không tồn tại");
  ...
  try {
    ...
    if (customer == null)
      return res.status(400).send("Customer này không tồn tại");
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (order) {
      ...
      let feedback_quantity =
        result && result.count ? parseInt(result.count) : 0;
...
let update = async (req, res, next) => {
  ...
  if (feedback_id === undefined)
    return res.status(400).send("Trường feedback_id không tồn tại");
  let rate = req.body.rate;
  if (rate === undefined)
    return res.status(400).send("Trường rate không tồn tại");
  let content = req.body.content;
...
let list = async (req, res, next) => {
  ...
  try {
    ...
    if (product == null)
      return res.status(400).send("Product này không tồn tại");
