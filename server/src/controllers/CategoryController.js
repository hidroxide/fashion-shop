...
let createLevel1 = async (req, res, next) => {
  try {
    ...
    if (title === undefined)
      return res.status(400).send("Trường title không tồn tại");
...
let createLevel2 = async (req, res, next) => {
  try {
    ...
    if (parent_id === undefined)
      return res.status(400).send("Trường parent_id không tồn tại");
    let parentCategory = await Category.findOne({
      where: { category_id: parent_id },
    });
    if (!parentCategory)
      return res.status(400).send("parent_id đã nhập không tồn tại");
    let category = await Category.findOne({
      where: { title: title, level: 2 },
    });
    if (category) return res.status(409).send("Tên danh mục đã tồn tại");
    else {
